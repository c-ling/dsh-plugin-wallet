...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var UNAVAILABLE_SETTINGS_SNAPSHOT = Object.freeze({
      status: "unavailable",
      value: undefined,
      base: undefined,
      user: undefined,
    ...
    var DICT = {
      zh: {
        ...
        navTooltip: "钱包：余额与用量",
        settingsNav: "钱包设置",
        settingsIntro: "配置低余额阈值、刷新周期与余额接口参数。",
        settingsCardTitle: "余额与刷新",
        thresholdLabel: "低余额阈值（元）",
        thresholdHint: "余额低于该值时侧边栏显示红色状态点。",
        refreshIntervalLabel: "余额刷新周期（毫秒）",
        refreshIntervalHint: "默认 300000（5 分钟），范围 30000–3600000。",
        balanceTimeoutLabel: "余额请求超时（毫秒）",
        balanceTimeoutHint: "默认 10000，范围 1000–120000。",
        balanceEndpointLabel: "余额接口地址",
        balanceEndpointHint: "通常保持官方地址即可。",
        settingsSave: "保存",
        settingsSaving: "保存中…",
        settingsSaved: "已保存",
        settingsReset: "恢复默认",
        settingsUnavailable: "当前环境未提供设置服务。",
        settingsLoading: "加载中…",
        settingsError: "保存失败：{error}",
        open: "打开钱包",
        close: "关闭",
        refresh: "刷新",
        refreshing: "刷新中…",
      ...
      en: {
        ...
        settingsNav: "Wallet settings",
        settingsIntro: "Configure low-balance threshold, refresh interval, and balance endpoint.",
        settingsCardTitle: "Balance & refresh",
        thresholdLabel: "Low balance threshold (CNY)",
        thresholdHint: "Shows the red sidebar dot when balance drops below this amount.",
        refreshIntervalLabel: "Balance refresh interval (ms)",
        refreshIntervalHint: "Default 300000 (5 min), range 30000–3600000.",
        balanceTimeoutLabel: "Balance request timeout (ms)",
        balanceTimeoutHint: "Default 10000, range 1000–120000.",
        balanceEndpointLabel: "Balance endpoint",
        balanceEndpointHint: "Usually keep the official endpoint.",
        settingsSave: "Save",
        settingsSaving: "Saving…",
        settingsSaved: "Saved",
        settingsReset: "Reset defaults",
        settingsUnavailable: "Settings service is not available in this environment.",
        settingsLoading: "Loading…",
        settingsError: "Save failed: {error}",
        open: "Open wallet",
        close: "Close",
        refresh: "Refresh",
    ...
    // No wallet-specific glyph ships in the rc.7 primitive set; the outline
    ...
    function WalletSettingsSection(props) {
      ...
      var scopeSubscribe = scope !== undefined ? scope.subscribe.bind(scope) : noopSubscribe;
      var scopeGetSnapshot = scope !== undefined ? scope.getSnapshot.bind(scope) : noopGetSnapshot;
      var snap = React.useSyncExternalStore(scopeSubscribe, scopeGetSnapshot, scopeGetSnapshot);

      var fallbackState = React.useState(null);
      var fallback = fallbackState[0];
      var setFallback = fallbackState[1];

      var thresholdState = React.useState("");
      var threshold = thresholdState[0];
      var setThreshold = thresholdState[1];
      var refreshState = React.useState("");
      var refreshIntervalMs = refreshState[0];
      var setRefreshIntervalMs = refreshState[1];
      var timeoutState = React.useState("");
      var balanceTimeoutMs = timeoutState[0];
      var setBalanceTimeoutMs = timeoutState[1];
      var endpointState = React.useState("");
      var balanceEndpoint = endpointState[0];
      var setBalanceEndpoint = endpointState[1];
      ...
      React.useEffect(function () {
        if (scope !== undefined && snap.status === "ready" && snap.value) {
          setThreshold(String(snap.value.threshold ?? ""));
          setRefreshIntervalMs(String(snap.value.refreshIntervalMs ?? ""));
          setBalanceTimeoutMs(String(snap.value.balanceTimeoutMs ?? ""));
          setBalanceEndpoint(String(snap.value.balanceEndpoint ?? ""));
          setSaved(false);
          setError(null);
          return;
        }
        if (scope === undefined || snap.status === "unavailable") {
          var alive = true;
          setFallback({ status: "loading", data: null, error: null });
          fetch("/dsh-plugin-wallet/config", { headers: { accept: "application/json" } })
            .then(function (response) {
              if (!response.ok) throw new Error("http-" + response.status);
              return response.json();
            })
            .then(function (json) {
              ...
              setFallback({ status: "ready", data: json.data, error: null });
              setThreshold(String(json.data.threshold ?? ""));
              setRefreshIntervalMs(String(json.data.refreshIntervalMs ?? ""));
              setBalanceTimeoutMs(String(json.data.balanceTimeoutMs ?? ""));
              setBalanceEndpoint(String(json.data.balanceEndpoint ?? ""));
              setSaved(false);
              setError(null);
            })
            .catch(function (err) {
              if (!alive) return;
              setFallback({ status: "error", data: null, error: String((err && err.message) || err) });
            });
          return function () {
            alive = false;
          };
      ...
      var usingFallback = scope === undefined || snap.status === "unavailable";

      if (scope !== undefined && snap.status === "loading") {
        return React.createElement("div", { className: "dsh-wallet-settings" }, t("settingsLoading"));
      }
      if (usingFallback) {
        if (!fallback || fallback.status === "loading") {
          return React.createElement("div", { className: "dsh-wallet-settings" }, t("settingsLoading"));
        }
        if (fallback.status === "error") {
          return React.createElement("div", { className: "dsh-wallet-settings" }, t("settingsError", { error: fallback.error }));
        }
      }
      ...
      var save = function () {
        ...
        setError(null);
        setSaved(false);
        if (!usingFallback && scope !== undefined) {
          settle(Promise.all([
            scope.set("threshold", Number(threshold)),
            scope.set("refreshIntervalMs", Number(refreshIntervalMs)),
            scope.set("balanceTimeoutMs", Number(balanceTimeoutMs)),
            scope.set("balanceEndpoint", String(balanceEndpoint || "https://api.deepseek.com/user/balance")),
          ]));
        } else {
          settle(fetch("/dsh-plugin-wallet/config", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({
              threshold: Number(threshold),
              refreshIntervalMs: Number(refreshIntervalMs),
              balanceTimeoutMs: Number(balanceTimeoutMs),
              balanceEndpoint: String(balanceEndpoint || "https://api.deepseek.com/user/balance"),
            }),
          }).then(function (response) {
            if (!response.ok) throw new Error("http-" + response.status);
            return response.json();
      ...
      var reset = function () {
        ...
        if (!usingFallback && scope !== undefined) {
          settle(Promise.all([
            scope.unset("threshold"),
            scope.unset("refreshIntervalMs"),
            scope.unset("balanceTimeoutMs"),
            scope.unset("balanceEndpoint"),
          ]));
        } else {
          settle(fetch("/dsh-plugin-wallet/config", {
            method: "POST",
      ...
      var field = function (label, hint, children) {
        return React.createElement(
          "label",
          { className: "dsh-wallet-settings-field" },
          React.createElement("span", { className: "dsh-wallet-settings-label" }, label),
          children,
          hint ? React.createElement("span", { className: "dsh-wallet-settings-hint" }, hint) : null,
        );
      };

      return React.createElement(
        "div",
        { className: "dsh-wallet-settings" },
        React.createElement(
          "div",
          { className: "dsh-wallet-settings-header" },
          React.createElement(
            "div",
            { className: "dsh-wallet-settings-header-row" },
            React.createElement("h2", { className: "dsh-wallet-settings-title" }, t("settingsNav")),
            saving ? React.createElement("span", { className: "dsh-wallet-settings-tag dsh-wallet-settings-tag-warn" }, t("settingsSaving")) : null,
            saved ? React.createElement("span", { className: "dsh-wallet-settings-tag dsh-wallet-settings-tag-ok" }, t("settingsSaved")) : null,
            error ? React.createElement("span", { className: "dsh-wallet-settings-tag dsh-wallet-settings-tag-error" }, t("settingsError", { error: error })) : null,
          ),
          React.createElement("p", { className: "dsh-wallet-settings-intro" }, t("settingsIntro")),
        ),
        React.createElement(
          "div",
          { className: "dsh-wallet-settings-content" },
          React.createElement(
            "div",
            { className: "dsh-wallet-settings-card" },
            React.createElement("h3", null, t("settingsCardTitle")),
            React.createElement(
              "div",
              { className: "dsh-wallet-settings-grid" },
              field(t("thresholdLabel"), t("thresholdHint"), React.createElement("input", {
                className: "dsh-wallet-settings-input",
                type: "number",
                min: 0,
                value: threshold,
                onChange: function (event) { setThreshold(event.target.value); },
              })),
              field(t("refreshIntervalLabel"), t("refreshIntervalHint"), React.createElement("input", {
                className: "dsh-wallet-settings-input",
                type: "number",
                min: 30000,
                max: 3600000,
                ...
                onChange: function (event) { setRefreshIntervalMs(event.target.value); },
              })),
              field(t("balanceTimeoutLabel"), t("balanceTimeoutHint"), React.createElement("input", {
                className: "dsh-wallet-settings-input",
                type: "number",
                min: 1000,
                max: 120000,
                step: 1000,
                value: balanceTimeoutMs,
                onChange: function (event) { setBalanceTimeoutMs(event.target.value); },
              })),
              field(t("balanceEndpointLabel"), t("balanceEndpointHint"), React.createElement("input", {
                className: "dsh-wallet-settings-input",
                type: "text",
                value: balanceEndpoint,
                onChange: function (event) { setBalanceEndpoint(event.target.value); },
              })),
            ),
          ),
        ),
        React.createElement(
          "div",
          { className: "dsh-wallet-settings-footer" },
          React.createElement("button", {
            type: "button",
            className: "dsh-wallet-settings-save",
            disabled: saving,
            onClick: save,
          }, saving ? t("settingsSaving") : t("settingsSave")),
          React.createElement("button", {
            type: "button",
            className: "dsh-wallet-settings-reset",
            disabled: saving,
            onClick: reset,
          }, t("settingsReset")),
        ),
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              rows.map(function (row) {
                ...
                var sessionTitle = row.item && row.item.displayTitle ? row.item.displayTitle : row.id;
                var sessionNameNode;
                if (typeof onOpenSession === "function") {
                  sessionNameNode = React.createElement(
                    "button",
                    {
                      type: "button",
                      className: "dsh-wallet-session-link",
                      onClick: function () { onOpenSession(row.id); },
                    },
                    sessionTitle,
                  );
                } else {
                  sessionNameNode = React.createElement("span", { className: "dsh-wallet-session-name" }, sessionTitle);
                }
                return React.createElement(
                  "tr",
                  { key: row.id },
                  React.createElement(
                    "td",
                    { className: "dsh-wallet-session-cell" },
                    sessionNameNode,
                    React.createElement("div", { className: "dsh-wallet-session-id dsh-wallet-muted" }, row.id),
                  ),
    ...
    function WalletPanel(props) {
      ...
      var openSession = function (id) {
        if (sessionsApi && typeof sessionsApi.open === "function") {
          sessionsApi.open(id);
        } else if (typeof props.open === "function") {
          props.open(id);
        }
        close();
      };
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(CostTable, { rows: rows, t: t, onOpenSession: openSession }),
          ),
        ),
      );
    }
    ...
    var css = [
      ...
      "/* 宽栏按钮完整复用设置触发的几何：1.0.7 下同样 42px、14px 字号、负外边距与圆角 */",
      ...
      ".dsh-wallet-wide{flex:none;width:calc(100% + 8px);height:42px;border-radius:12px;gap:8px;margin:0 -4px;padding:6px 10px;font-size:14px;line-height:22px;justify-content:flex-start;overflow:hidden;}",
      ".dsh-wallet-wide:hover{background:var(--dsw-alias-interactive-bg-hover);}",
      ".dsh-wallet-rail{position:relative;width:42px;height:42px;border-radius:50%;justify-content:center;gap:0;margin:8px 0 10px;padding:0;}",
      ...
      ".dsh-wallet-table td{padding:7px 8px 7px 0;border-bottom:1px solid var(--dsw-alias-border-l1);vertical-align:middle;}",
      ...
      ".dsh-wallet-session-cell{max-width:none;overflow-wrap:anywhere;word-break:break-word;}",
      ".dsh-wallet-session-link{box-sizing:border-box;display:block;max-width:100%;padding:0;border:none;background:transparent;color:inherit;font:inherit;text-align:left;cursor:pointer;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}",
      ".dsh-wallet-session-link:hover{color:var(--dsw-alias-label-primary);text-decoration:underline;}",
      ".dsh-wallet-session-name{display:block;max-width:100%;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}",
      ".dsh-wallet-session-id{display:block;max-width:100%;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}",
      ".dsh-wallet-model-cell{max-width:none;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}",
      ...
      ".dsh-wallet-settings{box-sizing:border-box;max-width:720px;width:100%;height:calc(100% + 24px);min-height:0;display:flex;flex-direction:column;gap:12px;margin-bottom:-24px;}",
      ".dsh-wallet-settings-header{flex:none;display:flex;flex-direction:column;gap:4px;padding-bottom:10px;border-bottom:1px solid var(--dsw-alias-border-l1);}",
      ".dsh-wallet-settings-header-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}",
      ".dsh-wallet-settings-title{margin:0;color:var(--dsw-alias-label-primary);font-size:16px;font-weight:500;line-height:24px;}",
      ".dsh-wallet-settings-intro{margin:0;color:var(--dsw-alias-label-tertiary);font-size:14px;line-height:22px;}",
      ".dsh-wallet-settings-tag{display:inline-flex;align-items:center;padding:1px 8px;border-radius:999px;font-size:11px;line-height:16px;font-weight:500;background:var(--dsw-alias-interactive-bg-hover-solid);}",
      ".dsh-wallet-settings-tag-ok{color:var(--dsw-alias-state-success-primary);}",
      ".dsh-wallet-settings-tag-warn{color:var(--dsw-alias-state-warn-primary);}",
      ".dsh-wallet-settings-tag-error{color:var(--dsw-alias-state-error-primary);}",
      ".dsh-wallet-settings-content{flex:1 1 auto;min-height:0;overflow-y:auto;display:flex;flex-direction:column;gap:14px;padding:2px 8px 8px 2px;margin-right:-8px;scrollbar-width:thin;}",
      ".dsh-wallet-settings-content::-webkit-scrollbar{width:8px;background:transparent;}",
      ".dsh-wallet-settings-content::-webkit-scrollbar-thumb{background:transparent;border-radius:4px;}",
      ".dsh-wallet-settings-content:hover::-webkit-scrollbar-thumb{background:var(--dsw-alias-scrollbar-bg-l2);}",
      ".dsh-wallet-settings-card{border:1px solid var(--dsw-alias-border-l2);background:transparent;border-radius:10px;padding:14px 16px;display:flex;flex-direction:column;gap:10px;}",
      ".dsh-wallet-settings-card h3{margin:0;font-size:14px;font-weight:600;color:var(--dsw-alias-label-primary);}",
      ".dsh-wallet-settings-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:10px 16px;}",
      ".dsh-wallet-settings-field{display:flex;flex-direction:column;gap:4px;min-width:0;font-size:13px;line-height:20px;}",
      ".dsh-wallet-settings-field > span:first-child{font-size:12px;color:var(--dsw-alias-label-secondary);}",
      ".dsh-wallet-settings-label{color:var(--dsw-alias-label-primary);font-weight:500;}",
      ".dsh-wallet-settings-hint{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px;}",
      ".dsh-wallet-settings-input{width:100%;box-sizing:border-box;padding:6px 8px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:inherit;}",
      ".dsh-wallet-settings-input:focus{outline:none;border-color:var(--dsw-alias-brand-primary);}",
      ".dsh-wallet-settings-footer{flex:none;display:flex;align-items:center;gap:10px;background:var(--dsw-alias-bg-layer-2);border-top:1px solid var(--dsw-alias-border-l1);padding:15px 2px 12px;}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-save,.dsh-wallet-settings-footer .dsh-wallet-settings-reset{flex:1 1 50%;min-width:0;height:40px;border-radius:20px;padding:0 12px;font-size:14px;font-weight:500;cursor:pointer;border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-primary);}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-save:hover:not(:disabled),.dsh-wallet-settings-footer .dsh-wallet-settings-reset:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l3);}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-save:disabled,.dsh-wallet-settings-footer .dsh-wallet-settings-reset:disabled{opacity:.4;cursor:not-allowed;}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-save{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);border-color:transparent;}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-save:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover);border-color:transparent;}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-reset{color:var(--dsw-alias-state-error-primary);}",
      ".dsh-wallet-settings-footer .dsh-wallet-settings-reset:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-danger);}",
      ".dsh-wallet-settings-saved{color:var(--dsw-alias-state-success-primary);font-size:12px;line-height:18px;}",
      ".dsh-wallet-settings-error{color:var(--dsw-alias-state-error-primary);font-size:12px;line-height:18px;}",
    ...
    function apply(ctx) {
      ...
      slots.inject("settings.section", function () {
        ...
        var scope = settingsScope !== undefined ? settingsScope.bind({ namespace: NS }) : null;
        return slots.register(
          {
