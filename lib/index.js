 ...
 *
 * Plain Cordis plugin (core path uses node builtins; optional 1.0.7 settings
 * integration lazily imports @deepseek-ai/dsh-settings when available):
 *
 ...
 *
 *      Harness 0.1.1-rc.1 compatibility: the projection registry now exposes
 *      only units that declare `wire` ({ viewSchema, view }) and parses
 *      cached rows through `stateSchema`, so the definition carries both
 *      (plus the legacy `schema`/`view` fields for older hosts). Session
 *      logs are zstd-packed multi-frame artifacts with no seekable suffix
 *      read, so every persisted session is served from the projection
 *      cache's zero-I/O `cachedSnapshot` rung; cold refolds are bounded per
 *      request (COLD_READ_BUDGET_PER_REQUEST) instead of refolding the whole
 *      corpus on every modal open.
 *
...
export const inject = ["webServer", "credentials", "settings", "llm", "sessionProjections", "sessionQuery", "sessionProjectionCache", "sessions"];
...
export const SESSION_COSTS_PATH = "/dsh-plugin-wallet/session-costs";
export const WALLET_SETTINGS_NS = "dsh-plugin-wallet";
...
// Optional 1.0.7 settings integration. Out-of-tree plugins cannot rely on
// profile node_modules being resolvable from the plugin path, so the import is
// attempted lazily; when the packages are absent the plugin keeps working with
// the entry config exactly as before.
...
function walletStateDir() {
  return join(dshHome(), "storages", "dsh-plugin-wallet");
}

function configFilePath() {
  return join(walletStateDir(), "config.json");
}

/** Fallback config persistence used when the optional settings packages are absent. */
export function readWalletConfigFile() {
  try {
...
export function writeWalletConfigFile(config) {
  const dir = walletStateDir();
...
/** Validate/normalize one stored token bucket; null when unusable. */
function readBucketField(value) {
  return typeof value === "number" && Number.isFinite(value) && value >= 0 ? value : 0;
}

function storedBucketOrZero(value) {
  if (typeof value !== "object" || value === null) return zeroBuckets();
  return {
    uncachedInputTokens: readBucketField(value.uncachedInputTokens),
    cacheReadTokens: readBucketField(value.cacheReadTokens),
    cacheWriteTokens: readBucketField(value.cacheWriteTokens),
    outputTokens: readBucketField(value.outputTokens),
  };
}

/**
 * `stateSchema.parse` for the 0.1.1-rc.1 projection registry: validates and
 * normalizes the folded internal state before it seeds a warm restore or a
 * client-visible view. Tolerant by design — missing optional fields are
 * filled with defaults so cached rows keep hitting the warm path; only a
 * non-object payload throws (the registry then discards the row).
 */
export function parseWalletState(value) {
  if (value === null || typeof value !== "object") {
    throw new Error(`walletSessionCost: invalid projection state (${typeof value})`);
  }
  const source = typeof value.buckets === "object" && value.buckets !== null ? value.buckets : {};
  const buckets = { legacy: {}, offpeak: {}, peak: {} };
  for (const period of ["legacy", "offpeak", "peak"]) {
    const periodSource = typeof source[period] === "object" && source[period] !== null ? source[period] : {};
    for (const [modelKey, bucket] of Object.entries(periodSource)) {
      if (typeof bucket === "object" && bucket !== null) {
        buckets[period][String(modelKey).slice(0, 256)] = storedBucketOrZero(bucket);
      }
    }
  }
  const headerSource = typeof value.currentModel === "object" && value.currentModel !== null ? value.currentModel : null;
  const currentModel = headerSource !== null && typeof headerSource.provider === "string" && typeof headerSource.model === "string"
    ? { provider: headerSource.provider.slice(0, 256), model: headerSource.model.slice(0, 256) }
    : null;
  const lastSource = typeof value.last === "object" && value.last !== null ? value.last : null;
  let last = null;
  if (lastSource !== null && (lastSource.category === "known" || lastSource.category === "unknown" || lastSource.category === "unsupported")) {
    if (typeof lastSource.bucket === "object" && lastSource.bucket !== null) {
      last = {
        turn: typeof lastSource.turn === "number" ? lastSource.turn : -1,
        step: typeof lastSource.step === "number" ? lastSource.step : -1,
        bucket: storedBucketOrZero(lastSource.bucket),
        category: lastSource.category,
        modelKey: typeof lastSource.modelKey === "string" ? lastSource.modelKey.slice(0, 512) : "",
        period: lastSource.period === "legacy" || lastSource.period === "offpeak" || lastSource.period === "peak" ? lastSource.period : "legacy",
      };
    }
  }
  return {
    currentModel,
    buckets,
    unknown: storedBucketOrZero(value.unknown),
    unsupported: storedBucketOrZero(value.unsupported),
    last,
  };
}

/** `viewSchema.parse` for the client-visible wallet cost view. */
export function parseWalletView(value) {
...
export const walletProjectionDefinition = Object.freeze({
  key: PROJECTION_KEY,
  stateVersion: 1,
  // 0.1.1-rc.1 contract: the registry parses cached rows through
  // `stateSchema` before seeding a warm restore; without it every cold read
  // crashes into the full-refold fallback.
  stateSchema: Object.freeze({ parse: parseWalletState }),
  schema: Object.freeze({ parse: parseWalletView }),
  init: () => ({
    currentModel: null,
  ...
  // 0.1.1-rc.1 contract: a unit without `wire` is host-only — its key is
  // omitted from snapshot()/viewCheckpoint()/restore() value sets, which is
  // exactly why the modal lost its data after the harness update. `wire.view`
  // produces the client value and `wire.viewSchema` gates it on the way out.
  wire: Object.freeze({
    viewSchema: Object.freeze({ parse: parseWalletView }),
    view: viewWalletProjection,
  }),
});
...
// ── all-session cost route helpers ─────────────────────────────────────────
...
function serviceOf(ctx, name) {
  ...
  if (typeof ctx.get === "function") {
    try {
      ...
      // Some Cordis service getters throw when accessed without inject; ignore.
    }
  }
  try {
    return ctx[name];
...
/**
 * Read one session's wallet projection using the same live/cold ladder as the
 * built-in session list (0.1.1-rc.1 shape): live sessions use the registry
 * snapshot, persisted sessions use the projection cache's zero-I/O listing
 * read (`cachedSnapshot`, identity-checked stored rows — no log loads), and
 * only a cache miss falls back to the cold read or a raw log replay.
 *
 * The cold rungs are O(full log) on the zstd-packed storage the new harness
 * ships (no seekable suffix read), so `options.coldBudget` bounds how many
 * full refolds one route request may trigger; exhausted budgets skip the row
 * instead of hanging the modal, and the cache write-behind converges the
 * rest on later requests.
 */
export async function sessionCostSnapshot(ctx, id, signal, options = {}) {
  const sessions = serviceOf(ctx, "sessions");
  const sessionProjections = serviceOf(ctx, "sessionProjections");
  const live = typeof sessions?.get === "function" ? sessions.get(id) : undefined;
  if (live !== undefined && typeof sessionProjections?.snapshot === "function") {
    try {
      return sessionProjections.snapshot(live);
    } catch (error) {
      // Fall through to cold/persisted reads when the live snapshot is unavailable.
    }
  }
  const cache = serviceOf(ctx, "sessionProjectionCache");
  // Zero-I/O rung: whole values straight from the stored rows. This is the
  // listing read the cache exists for; the wallet row must carry `wire` in
  // the projection definition or it never appears in `values`.
  const header = options.header;
  if (header !== undefined && typeof cache?.cachedSnapshot === "function") {
    try {
      const cached = cache.cachedSnapshot(header);
      if (cached?.values?.[PROJECTION_KEY] !== undefined) return cached;
    } catch (error) {
      // Fall through to the cold read when the listing read is unavailable.
    }
  }
  // Slow rungs: both refold the whole stored log on the zstd backend, so they
  // share the caller's per-request budget.
  const budget = options.coldBudget;
  ...
  if (typeof cache?.coldSnapshot === "function") {
    ...
    try {
      return await cache.coldSnapshot(id, signal);
    } catch (error) {
      // Fall through to a direct log replay if the projection cache cannot serve it.
    }
  }
  ...
  if (typeof query?.readSession === "function") {
    ...
    for (const event of loaded.events) state = applyWalletProjectionEvent(state, event);
    return {
      values: {
        [PROJECTION_KEY]: walletProjectionDefinition.schema.parse(viewWalletProjection(state)),
      },
    };
  }
  return null;
...
/**
 * Collect wallet costs for every logical session (live + persisted). This is
 * the data source for the wallet modal's cost estimate and all-session ranking,
 * so sessions created before the wallet plugin was installed are included too.
 *
 * Persisted sessions read through the zero-I/O `cachedSnapshot` rung; at most
 * `COLD_READ_BUDGET_PER_REQUEST` cache misses refold per request, so a cold
 * or wiped projection cache degrades to a partial ranking that converges on
 * later requests instead of a multi-minute hang.
 */
export const COLD_READ_BUDGET_PER_REQUEST = 8;

export async function collectSessionCostRows(ctx, signal) {
  const query = serviceOf(ctx, "sessionQuery");
  const sessions = serviceOf(ctx, "sessions");
  const entries = [];
  if (typeof query?.listSessions === "function") {
    try {
      ...
      for (const record of records) {
        if (record?.header?.id) entries.push({ id: record.header.id, header: record.header });
      }
    } catch (error) {
      ctx.logger?.warn?.(`dsh-plugin-wallet: session list failed, falling back to live sessions: ${String(error?.message ?? error)}`);
    }
  }
  if (entries.length === 0 && typeof sessions?.list === "function") {
    try {
      for (const session of sessions.list()) {
        if (session?.id) entries.push({ id: session.id, header: session.header });
      }
    } catch (error) {
      ctx.logger?.warn?.(`dsh-plugin-wallet: live session list failed: ${String(error?.message ?? error)}`);
    }
  }
  if (entries.length === 0) return [];

  const rows = [];
  const coldBudget = { remaining: COLD_READ_BUDGET_PER_REQUEST };
  ...
  const worker = async () => {
    while (cursor < entries.length) {
      ...
      cursor += 1;
      try {
        const snapshot = await sessionCostSnapshot(ctx, entry.id, signal, { header: entry.header, coldBudget });
        const cost = snapshot?.values?.[PROJECTION_KEY];
        if (cost !== undefined && cost !== null && totalOfBucket(cost.totalTokens) > 0) {
          rows.push({ id: entry.id, cost });
        }
      } catch (error) {
        ctx.logger?.warn?.(`dsh-plugin-wallet: session cost for "${entry.id}" failed: ${String(error?.message ?? error)}`);
      }
      if (!budgetWarned && coldBudget.remaining <= 0 && cursor < entries.length) {
        budgetWarned = true;
        ctx.logger?.warn?.(`dsh-plugin-wallet: cold-refold budget exhausted after ${COLD_READ_BUDGET_PER_REQUEST} sessions; ranking is partial until the projection cache warms up`);
      }
    }
  };
...
export function apply(ctx, config = {}) {
  ...
  // Local/link development may not have the optional settings packages
  // resolvable from the plugin path. Fall back to the classic storage file so
  // the Settings page still works without a settings provider.
  if (settingsApi === null) {
    const saved = readWalletConfigFile();
    if (saved !== null) current = saved;
  }


  // 1.0.7: register the plugin-owned settings namespace when the optional
  // settings/schemastery packages are available. installSettingsSection uses
  // the entry config as the composition base and falls back to it when no
  // settings provider is mounted.
  if (settingsApi !== null && schemastery !== null && typeof ctx.inject === "function") {
    const NS = settingsApi.settingsNamespace(WALLET_SETTINGS_NS);
    const SCHEMA = schemastery.object({
      threshold: schemastery.number().min(0).max(1_000_000).default(DEFAULT_CONFIG.threshold),
      refreshIntervalMs: schemastery.number().min(30_000).max(3_600_000).default(DEFAULT_CONFIG.refreshIntervalMs),
      balanceTimeoutMs: schemastery.number().min(1_000).max(120_000).default(DEFAULT_CONFIG.balanceTimeoutMs),
      balanceEndpoint: schemastery.string().default(DEFAULT_CONFIG.balanceEndpoint),
    });
    try {
      settingsApi.installSettingsSection(ctx, NS, SCHEMA, current, {
        setSource: (next) => {
          currentSource = next;
          current = next();
        },
      ...
      ctx.logger?.warn?.(`dsh-plugin-wallet: settings integration unavailable: ${String(error?.message ?? error)}`);
    }
  }
  ...
  if (ctx.sessionProjections?.register !== undefined) {
    try {
      ...
      ctx.logger?.warn?.(`dsh-plugin-wallet: projection registration failed: ${String(error?.message ?? error)}`);
    }
  }
  ...
  const load = async (force) => {
    ...
    inflight = (async () => {
      ...
      try {
        const facts = await collectRouteFacts(ctx, current);
        ...
        const balance = await requestBalance(current.balanceEndpoint, facts.apiKey, current.balanceTimeoutMs);
        return {
          status: "ok",
          fetchedAt: startedAt,
          stale: false,
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        if (req.method === "POST") {
          ...
          for await (const chunk of req) {
            ...
            if (body.length > 64 * 1024) {
              sendJson(res, 413, { ok: false, error: { code: "payload-too-large" } });
              return;
            }
          }
          let patch = {};
          try {
            ...
            sendJson(res, 400, { ok: false, error: { code: "bad-json" } });
            return;
          }
          const next = normalizeConfig({ ...current, ...patch });
          writeWalletConfigFile(next);
          current = next;
          sendJson(res, 200, { ok: true, data: current });
          return;
        }
        ...
        ctx.logger?.error?.(`dsh-plugin-wallet: config route failed: ${String(error?.message ?? error)}`);
        sendJson(res, 500, { ok: false, error: { code: "internal" } });
      }
    },
  }), "dsh-plugin-wallet: config route");
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        ctx.logger?.error?.(`dsh-plugin-wallet: session costs route failed: ${String(error?.message ?? error)}`);
        sendJson(res, 500, { ok: false, error: { code: "internal", message: String(error?.message ?? error) } });
      }
    },
  }), "dsh-plugin-wallet: session costs route");
}
