 ...
 * built-in session list: live sessions use the registry snapshot, persisted
 * sessions use the projection cache's cold read (which refolds and writes back
 * when the wallet key is missing), and a final fallback replays the raw log.
 */
export async function sessionCostSnapshot(ctx, id, signal) {
...
export async function collectSessionCostRows(ctx, signal) {
  ...
  const worker = async () => {
    while (cursor < ids.length) {
      ...
      cursor += 1;
      try {
        const snapshot = await sessionCostSnapshot(ctx, id, signal);
        const cost = snapshot?.values?.[PROJECTION_KEY];
        if (cost !== undefined && cost !== null && totalOfBucket(cost.totalTokens) > 0) {
          rows.push({ id, cost });
        }
      } catch (error) {
        ctx.logger?.warn?.(`dsh-plugin-wallet: session cost for "${id}" failed: ${String(error?.message ?? error)}`);
      }
    }
  };
